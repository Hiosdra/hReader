...
class ArticleTtsPlaybackService : Service() {
    ...
    private val player: ArticleTtsPlayer by inject()
    ...
    override fun onCreate() {
        ...
        mediaSession = MediaSession(this, getString(R.string.notification_tts_title)).apply {
            setCallback(object : MediaSession.Callback() {
                override fun onPlay() = this@ArticleTtsPlaybackService.player.resume()

                override fun onPause() = this@ArticleTtsPlaybackService.player.pause()

                override fun onStop() = this@ArticleTtsPlaybackService.player.stop()
            })
    ...
    private fun startPlayback() {
        if (!foregroundStarted) {
            ...
            ServiceCompat.startForeground(
                this,
                NOTIFICATION_ID,
                notification(player.state.value),
                ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK
            )
            mediaSession.isActive = true
            observeState()
        }
        updateState(player.state.value)
    }

    private fun observeState() {
        stateJob = serviceScope.launch {
            player.state.collectLatest(::updateState)
        }
    }
