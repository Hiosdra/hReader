...
Secrets are kept in their own `hreader_secrets` preferences file, which `backup_rules.xml` and
`data_extraction_rules.xml` exclude from cloud backup and device transfer; everything else in
`hreader_prefs` is restored. A new secret goes in that file and in `SECRET_KEYS`.

Release builds permit no cleartext traffic. The debug variant overrides
`res/xml/network_security_config.xml` to allow it, which is where a plain-HTTP server on the local
network belongs.
...
- The article list is Paging 3 over a Room `PagingSource`. Nothing may load the whole list into
  memory again: totals are counted in SQL, and filtering and search belong in the query.
- The Glance widget composes in the launcher's process and cannot touch the database. Everything
  it shows is written ahead of time by `UnreadWidgetUpdater`, which every change to the unread set
  has to notify.
- `app/src/release/generated/baselineProfiles/` is generated by
  `./gradlew :app:generateReleaseBaselineProfile` against a connected device, and is committed.
