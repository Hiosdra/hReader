...
hReader is an Android RSS/Feed reader app built with Kotlin, Jetpack Compose, and MVVM architecture. It connects to a FreshRSS backend (Google Reader compatible API) or a Miniflux backend to synchronize feeds and articles.
...
- Backend choice, server address and credentials are stored in `PreferencesManager` and edited in Settings
